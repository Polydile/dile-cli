...
function processTemplatesRecursive(srcDir, destDir, templateData) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      // Files that should not have the prefix replaced (keep 'dile' prefix)
      const excludedFromPrefixReplace = ['dile.config.js'];

      // Replace 'dile' prefix with the custom appPrefix (except for excluded files)
      if (outputFileName.startsWith('dile') && !excludedFromPrefixReplace.includes(outputFileName)) {
