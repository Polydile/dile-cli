...
export function registerCommands(program) {
  program
    ...
    .option('--force', 'Overwrite existing app folder')
    .action(wrapAction(createApp));

  program
    .command('config-init')
    .description('Initialize a dile configuration file')
    .option('--cjs', 'Generate CommonJS format (dile.config.cjs)')
    .option('--mjs', 'Generate ES Module format (dile.config.mjs)')
    .option('--json', 'Generate JSON format (dile.config.json)')
    .option('--prefix <prefix>', 'Component prefix (e.g. dile)')
  ...
  program
    ...
    .option('--force', 'Overwrite existing files')
    .option('--belongs <name>', 'Generates the belongsTo and relationId properties sometimes required by the <dile-crud-insert> component')
