...
export const generateResourceConfig = createTemplateCommand({
  ...
  buildTemplateData: ({ name, subpath }, options) => {
    ...
    const itemComponent =
      (typeof options.itemComponent === 'string' && options.itemComponent.trim()) ||
      (typeof options.item_component === 'string' && options.item_component.trim()) ||
      undefined;

    const detailComponent =
      (typeof options.detailComponent === 'string' && options.detailComponent.trim()) ||
      (typeof options.detail_component === 'string' && options.detail_component.trim()) ||
      undefined;
