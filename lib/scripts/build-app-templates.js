...
// Carpetas que se copian sin modificar (sin agregar .hbs)
const COPY_AS_IS = [
  'public'
];
...
function copyDirRecursive(src, dest, copyAsIs = false) {
  ...
  entries.forEach((entry) => {
    ...
    const isInCopyAsIsFolder = copyAsIs || COPY_AS_IS.includes(entry.name);

    if (entry.isDirectory()) {
      const destPath = path.join(dest, destFileName);
      console.log(`📁 Copiando carpeta: ${entry.name}`);
      copyDirRecursive(sourcePath, destPath, isInCopyAsIsFolder);
    } else {
      // Solo agregar .hbs si NO estamos en una carpeta "copy as is"
      if (!copyAsIs) {
